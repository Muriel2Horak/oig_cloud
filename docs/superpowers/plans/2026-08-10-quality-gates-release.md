...
- [ ] Run install/build with explicit Vite production mode, clean temporary HOME, and a fail-closed environment allowlist. Reject `.env*`, project/user `.npmrc`, `VITE_*`, `NODE_OPTIONS`, and unapproved `NPM_CONFIG_*`; tests inject each source and prove no unchanged build ID can accompany changed bytes.
