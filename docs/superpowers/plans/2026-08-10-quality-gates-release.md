...
- [ ] PR workflow runs after required build/tests, names artifact `oig-cloud-<full-sha>`, uploads archive/manifest/digest, and never commits generated bytes.
- [ ] Support `workflow_dispatch` for the exact currently deployed reviewed SHA so the first HP rollout has a verified previous artifact.
...
- [ ] RED tests: missing artifact/digest, mismatch, manifest-SHA mismatch, traversal/symlink, unexpected file, corrupt gzip, and local-source invocation fail before mount/copy/restart.
...
- [ ] Validate archive members before extraction; reject absolute/traversal/symlink entries. Verify every extracted file digest and no extras.
- [ ] Record current and previous reviewed commit/archive digests in the remote deployment manifest. Refuse rollout unless a locally available verified previous artifact is named for rollback.
- [ ] Rollback command is the same verified deploy command using the retained previous archive/digest/commit; add dry-run and manifest readback.
- [ ] Test copy failure/restart failure: retain prior manifest and report exact recovery command without deleting the previous artifact.
...
- [ ] Deploy PR-head artifact to HP through dry-run then verified deploy. Retain the printed rollback command.
