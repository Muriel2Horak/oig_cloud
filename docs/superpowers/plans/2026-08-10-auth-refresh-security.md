...
- [ ] Create distinct empty mode-`0600` user/global npmrc files under the private build temp directory. Point `NPM_CONFIG_USERCONFIG` and `NPM_CONFIG_GLOBALCONFIG` at their respective absolute paths, assert npm reports those two different paths before install, and reject parent overrides. Never reuse one file for both levels; host `/etc/npmrc` and Homebrew/global npmrc are not consulted.
