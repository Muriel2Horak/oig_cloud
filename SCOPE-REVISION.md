...
- **Named transform-executability rule:** `TER-1 Transform Executability Rule`. Every registered migration transform MUST be either (a) synchronous, CPU-only, no disk/network/HA state access, and `<= 5 ms` on the CI fixture, or (b) declared async/executor-backed and invoked through an awaited coroutine or `hass.async_add_executor_job` outside the event loop hot path. Test: register a transform fixture that attempts blocking I/O or returns an awaitable from the sync registry; the migration test must fail unless the transform is routed through the async/executor path. Justification: this closes the R2/R6/R7/R8 open risk where a sync `register_transform(fn)` can silently become blocking under `async_setup_entry`.
