...
export class BatteryChartModule {
    ...
    async loadAndRender() {
        ...
        const series = this.buildSeries(timelineData);
        const datasets = this.buildDatasets(series);

        this.renderChart(series.labels, datasets, { minCapacityKwh, maxCapacityKwh });

        console.log('[BatteryChart] Chart rendered successfully');
    }

    buildSeries(timelineData) {
        const labels = [];
    ...
    buildDatasets(series) {
        ...
        datasets.push({
            ...
            data: series.batteryCapacityData,
            borderColor: '#00bcd4',
        ...
        if (series.gridChargeData.some(v => v > 0)) {
            datasets.push({
                label: 'Nabíjení ze sítě',
                data: series.gridChargeData,
        ...
        if (series.solarChargeData.some(v => v > 0)) {
            datasets.push({
                label: 'Nabíjení ze soláru',
                data: series.solarChargeData,
    ...
    renderChart(labels, datasets, limits) {
        if (this.chart) {
        ...
        this.chart = new Chart(this.canvasElement, {
            ...
            options: this.buildChartOptions(limits)
        });
    }

    buildChartOptions(limits) {
        return {
            responsive: true,
            ...
            scales: {
                ...
                'y-battery': {
                    type: 'linear',
                    position: 'left',
                    min: limits?.minCapacityKwh,
                    max: limits?.maxCapacityKwh,
                    ticks: {
                        color: '#00bcd4',
