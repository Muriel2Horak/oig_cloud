...
export class PricingChartModule {
    ...
    async loadAndRender() {
        ...
        const pricing = this.buildPriceDatasets(timeline);
        const datasets = [...pricing.datasets];
        const allLabels = pricing.labels;

        datasets.push(...this.buildSolarDatasets(allLabels));
        datasets.push(...this.buildBatteryDatasets(allLabels));

        this.renderChart(allLabels, datasets);
    ...
    buildPriceDatasets(timeline) {
        const labels = timeline.map(p => new Date(p.timestamp));
    ...
    buildSolarDatasets(allLabels) {
        ...
        const hasString1 = string1Data.some(v => v !== null && v !== undefined && v > 0);
        const hasString2 = string2Data.some(v => v !== null && v !== undefined && v > 0);
    ...
    buildBatteryDatasets(allLabels) {
        const datasets = [];
        ...
        if (solarAddData.some(v => v !== null && v !== undefined)) {
            datasets.push({
        ...
        if (gridAddData.some(v => v !== null && v !== undefined)) {
            datasets.push({
    ...
    renderChart(allLabels, datasets) {
        if (this.chart) {
    ...
    isLightTheme() {
        try {
            ...
            console.debug('[PricingChart] Theme detection failed:', err);
        }
        return false;
    }
}
