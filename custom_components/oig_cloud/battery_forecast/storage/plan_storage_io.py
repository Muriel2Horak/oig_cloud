...
async def load_plan_from_storage(
    ...
    if not sensor._plans_store:
        _LOGGER.error("[OIG_CLOUD_ERROR][component=planner][corr=na][run=na] " + STORAGE_HELPER_NOT_INITIALIZED)
    ...
    try:
        ...
        _LOGGER.error("[OIG_CLOUD_ERROR][component=planner][corr=na][run=na] " + "Error loading plan from Storage: %s", err, exc_info=True)
...
async def save_plan_to_storage(
    ...
    try:
        ...
        _LOGGER.error("[OIG_CLOUD_ERROR][component=planner][corr=na][run=na] " + "Error saving plan to Storage: %s", err, exc_info=True)
...
def _get_cached_plan(sensor: Any, date_str: str, reason: str) -> Optional[Dict[str, Any]]:
    ...
    if cached:
        _LOGGER.warning("[OIG_CLOUD_WARNING][component=planner][corr=na][run=na] " + "Using in-memory cached plan for %s (%s)", date_str, reason)
        return cached
    return None
...
def _cache_plan_in_memory(
    ...
    _LOGGER.warning("[OIG_CLOUD_WARNING][component=planner][corr=na][run=na] " + "Stored plan in memory cache (Storage failed): date=%s, intervals=%s", date_str,
    len(intervals),)
...
def _schedule_retry_save(sensor: Any, date_str: str) -> None:
    ...
    async def retry_save(_now):
        ...
        if success:
            ...
            _LOGGER.warning("[OIG_CLOUD_WARNING][component=planner][corr=na][run=na] " + "Retry failed for %s", date_str)
...
async def plan_exists_in_storage(sensor: Any, date_str: str) -> bool:
    ...
    try:
        ...
        _LOGGER.error("[OIG_CLOUD_ERROR][component=planner][corr=na][run=na] " + "Error checking plan existence: %s", err, exc_info=True)
        return False
