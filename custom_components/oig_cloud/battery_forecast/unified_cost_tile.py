...
async def build_unified_cost_tile(
    ...
    try:
        today_data = await _build_today_cost_data(self)
    ...
    try:
        yesterday_data = _get_yesterday_cost_from_archive(self, mode_names=mode_names)
    except Exception as e:
    ...
    try:
        tomorrow_data = await _build_tomorrow_cost_data(self, mode_names=mode_names)
    except Exception as e:
...
def _build_baseline_comparison(sensor: Any, hybrid_cost: float) -> Dict[str, Any]:
...
async def _analyze_yesterday_performance(sensor: Any) -> str:
...
async def _analyze_tomorrow_plan(sensor: Any) -> str:
...
async def _build_today_cost_data(sensor: Any) -> Dict[str, Any]:  # noqa: C901
    ...
    baseline_comparison = _build_baseline_comparison(self, plan_total)

    today_tooltip = _analyze_today_variance(self, intervals, plan_total, eod_predicted)
    yesterday_tooltip = await _analyze_yesterday_performance(self)
    tomorrow_tooltip = await _analyze_tomorrow_plan(self)
...
def _get_yesterday_cost_from_archive(
    sensor: Any, *, mode_names: Optional[Dict[int, str]] = None
) -> Dict[str, Any]:
    ...
    if yesterday in self._daily_plans_archive:
        ...
        plan_total = sum(
            _resolve_interval_cost(interval, prefer_actual=False)
            for interval in archive_data.get("plan", [])
        )
        actual_total = sum(
            _resolve_interval_cost(interval, prefer_actual=True)
...
async def _build_tomorrow_cost_data(
    sensor: Any, *, mode_names: Optional[Dict[int, str]] = None
) -> Dict[str, Any]:
