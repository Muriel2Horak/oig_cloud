...
def _summarize_guard_reason(
    guard_metrics: Dict[str, Any], block: Dict[str, Any]
) -> Optional[str]:
    ...
    forced_mode = guard_metrics.get("guard_forced_mode") or block.get("mode_planned")
...
def _resolve_charge_kwh(
    avg_grid_charge: Optional[float], delta_kwh: Optional[float]
) -> Optional[float]:
    if avg_grid_charge is not None and avg_grid_charge > 0.01:
        return avg_grid_charge
    if delta_kwh is not None and delta_kwh > 0.05:
        return delta_kwh
    return None


def _summarize_ups_mode(
    avg_price: Optional[float],
    max_ups_price: float,
...
def _summarize_home2_mode(
    avg_home1_saving: Optional[float], avg_recharge_cost: Optional[float]
) -> Optional[str]:
...
def _summarize_home3_mode(
    avg_solar: Optional[float], avg_load: Optional[float]
) -> Optional[str]:
...
def _select_data_type(tab_name: str) -> str:
    if tab_name == "yesterday":
        return "completed"
    if tab_name == "today":
        return "both"
...
def build_mode_blocks_for_tab(  # noqa: C901
    ...
    *,
    ...
    data_type = _select_data_type(tab_name)
