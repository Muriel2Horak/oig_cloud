    ...
    def _simulate_mode_behavior(
        ...
        if mode == HOME_I:
            ...
            if net_energy > 0:
                ...
                available_capacity = (
                    self.context.battery_capacity_kwh - battery_before_kwh
                )
                charge_power = min(
                    net_energy, available_capacity, self._get_max_charge_15min()
                )
                ...
                available_discharge = max(
                    0, battery_before_kwh - self.context.min_capacity_kwh
                )
            ...
            if solar_kwh > 0:
                ...
                charge_power = min(
                    solar_kwh, available_capacity, self._get_max_charge_15min()
                )
                battery_charge = charge_power
            ...
            if solar_kwh > 0:
                ...
                if remaining_solar > 0:
                    boiler_solar, export_solar = self._distribute_surplus(
                        remaining_solar
                    )
                    boiler += boiler_solar
