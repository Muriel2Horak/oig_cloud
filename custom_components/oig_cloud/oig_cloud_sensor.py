...
from .oig_cloud_sensor_runtime import OigCloudSensorRuntimeMixin
from .oig_cloud_sensor_setup import get_sensor_definition, resolve_box_id
...
class OigCloudSensor(OigCloudSensorRuntimeMixin, CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator: OigCloudCoordinator, sensor_type: str) -> None:
        ...
        _LOGGER.debug("Initialized sensor %s with box_id: %s", sensor_type, self._box_id)

        sensor_def = get_sensor_definition(sensor_type)

        if sensor_type.startswith("service_shield"):
            _LOGGER.warning("🔍 ServiceShield %s definition: %s", sensor_type, sensor_def)
