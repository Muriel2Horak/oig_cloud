...
CHMU_CAP_URL = "https://www.chmi.cz/files/portal/docs/meteo/om/bulletiny/XOCZ50_OKPR.xml"
...
class ChmuApi:
    ...
    def _parse_info_block(
        self, 
        alert_elem: ET.Element, 
        info_elem: ET.Element
        ...
        if severity_level == 0:
            awareness_level = self._get_text(
                info_elem, 
                "parameter[valueName='awareness_level']/value",
                ""
            )
            ...
            if lon > min(p1_lon, p2_lon):
                if lon <= max(p1_lon, p2_lon):
                    if lat <= max(p1_lat, p2_lat):
                        ...
                        if p1_lat == p2_lat or (x_intersection and lat <= x_intersection):
                            inside = not inside
        ...
        try:
            ...
            result = {
                ...
                "highest_severity_cz": max(
                    (a.get("severity_level", 0) for a in all_alerts), 
                    default=0
                ),
                "gps_location": {
                    "latitude": latitude,
