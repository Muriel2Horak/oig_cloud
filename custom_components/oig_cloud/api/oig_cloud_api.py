...
# Funkce na nastavení ručního ohřevu Bojleru    
    async def set_boiler_mode(self, enabled: bool) -> bool:
        with tracer.start_as_current_span("set_boiler_mode") as span:
            try:
                self._logger.debug(f"Setting boiler mode to {enabled}")
                async with self.get_session() as session:
                    data = json.dumps(
                        {
                            "id_device": self.box_id,
                    ...
                    self._logger.debug(
                        f"Sending boiler to manual request to {target_url} with {data.replace(self.box_id, 'xxxxxx')}"
                    )
                    with tracer.start_as_current_span(
                        "set_mode.post",
                        kind=SpanKind.SERVER,
