...
async def intercept_service_call(
    ...
    if shield.running is not None:
        ...
        _LOGGER.info(
            "[OIG Shield] Spouštím službu %s (fronta prázdná)", service_name
        )
...
async def start_call(
    ...
    try:
        ...
        if coordinator:
            ...
            _LOGGER.debug("[OIG Shield] Coordinator refreshnut - entity by měly být aktuální")
        else:
            _LOGGER.warning(
...
async def safe_call_service(shield: Any, service_name: str, service_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if "entity_id" in service_data:
            ...
            if service_name == "set_boiler_mode":
                ...
                for boiler_entity in boiler_entities:
                    if shield._check_entity_state_change(
                        boiler_entity, expected_value
                    ):
                        shield._logger.info(
                            "✅ Boiler mode změněn na %s", mode_value
                        )
                        return True
        ...
        shield._logger.error(
            "❌ Chyba při volání služby %s: %s", service_name, err
        )
        return False
...
async def log_event(
    ...
    if event_type == "queued":
        ...
        if is_limit_change:
            message = f"Dokončeno – {friendly_name}: limit nastaven na {expected_value}W"
        else:
