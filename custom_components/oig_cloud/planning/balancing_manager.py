...
class BalancingConfig:
    ...
    def validate(self) -> None:
        ...
        if self.opportunistic_threshold_soc < 90.0 or self.opportunistic_threshold_soc > 100.0:
...
class BalancingManager:
    ...
    async def check_economic_balancing(self) -> Optional[str]:
        ...
        if not is_valid:
            self._logger.info("Mediána validation failed - economic balancing not triggered")
            return None
    ...
    async def _get_export_window(self) -> Optional[Tuple[datetime, datetime, List[float]]]:
