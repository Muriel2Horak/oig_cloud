...
class WeatherMonitor:
    ...
    async def _check_weather_warning(self) -> None:
        ...
        # Warning detected
        if self.config.emergency_levels and warning_state in self.config.emergency_levels:
    ...
    async def _activate_emergency(
        ...
        if warning_end_attr:
            try:
                ...
                duration_hours = int((warning_end - self._warning_start).total_seconds() / 3600)
            except Exception as e:
    ...
    async def _update_emergency_plan(self) -> None:
        ...
        warning_state = sensor.state.lower()
        if not self.config.emergency_levels or warning_state not in self.config.emergency_levels:
            # Warning ended
        ...
        if warning_end_attr:
            try:
                warning_end = datetime.fromisoformat(warning_end_attr)
                remaining_hours = max(1, int((warning_end - now).total_seconds() / 3600))
            except Exception as e:
