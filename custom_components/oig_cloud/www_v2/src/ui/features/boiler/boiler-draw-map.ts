...
export class OigBoilerDrawMap extends LitElement {
  ...
  private _heatmapSvg(): string {
    ...
    const W = 940, padL = 70, padR = 52, rowH = 20, gap = 4, padT = 8;
    const plotW = W - padL - padR, bw = plotW / SLOTS;
    const H = padT + days.length * (rowH + gap) + 14;
    const col = (v: number) => {
    ...
    for (let h = 0; h <= 24; h += 3) {
      ...
      s += `<text x="${xx}" y="${H - 2}" fill="#6b7785" font-size="9" text-anchor="middle">${String(h).padStart(2, '0')}</text>`;
    }
    days.forEach((d, r) => {
      const y = padT + r * (rowH + gap);
      s += `<text x="${padL - 6}" y="${y + rowH / 2 + 4}" fill="#9fb0c0" font-size="10" text-anchor="end">${dayLabel(d.date, this.lang)}</text>`;
      ...
      s += `<text x="${padL + plotW + 6}" y="${y + rowH / 2 + 4}" fill="#cfe0ee" font-size="11" font-weight="600">${Math.round(d.totalLiters)} L</text>`;
    });
    s += '</svg>';
    return s;
  }
  ...
  private _profileSvg(prof: number[]): string {
    const W = 940, H = 214, padL = 30, padR = 14, padT = 32, padB = 24;
    ...
    for (let h = 0; h <= 24; h += 3) {
      const xx = x(h * 4);
      s += `<line x1="${xx}" y1="${padT}" x2="${xx}" y2="${padT + plotH}" stroke="rgba(255,255,255,.05)"/><text x="${xx}" y="${H - 7}" fill="#6b7785" font-size="9" text-anchor="middle">${String(h).padStart(2, '0')}</text>`;
    }
    for (let i = 0; i < SLOTS; i++) {
