...
class BatteryCapacityTracker:
    ...
    def __init__(
        ...
        # Bezpečnostní limity pro sanity check - zpřísněno pro realističtější výsledky
        # SoH nad 100% je fyzicky nemožné, pod 70% je extrémní degradace
        self._max_soh_percent = 100.0  # Sníženo z 105%
        self._min_soh_percent = 70.0  # Zvýšeno z 60%
        # Maximální rozumná roční degradace je ~5% (lithiové baterie)
        self._max_degradation_percent_per_year = 15.0  # Sanity limit
    ...
    async def analyze_history_for_cycles(
        ...
        # ZJEDNODUŠENO: Používáme _month senzory které se neresetují o půlnoci
    ...
    async def _calculate_capacity_from_energy(
        ...
        """
        ZJEDNODUŠENÝ ALGORITMUS: Spočítat kapacitu z energy sensorů (_month).

        Logika:
        1. Vezmi charge_month[konec] - charge_month[začátek] = nabitá energie
        2. Ověř že discharge_month se během intervalu nezvýšila (= čisté nabíjení)
        3. Kapacita = nabitá energie / delta_soc
        ...
        Args:
            ...
            end_soc: Koncový SoC
            charge_energy_sensor: ID nabíjecího energy sensoru (_month)
            discharge_energy_sensor: ID vybíjecího energy sensoru (_month)
        ...
        try:
            ...
            _LOGGER.debug(
                f"Analyzing cycle {start_time} → {end_time}, ΔSoC={delta_soc:.1f}%"
            )

            # Načíst energy history pro _month senzory
            ...
            # Získat hodnoty na začátku a konci intervalu
            ...
            if (
                ...
                _LOGGER.debug("Missing energy values for cycle")
                return None

            # ZJEDNODUŠENÁ LOGIKA:
            # 1. Spočítat nabíjecí energii
            total_charge_wh = charge_end - charge_start

            # 2. Detekovat reset měsíce (charge_month klesla = nový měsíc)
            if total_charge_wh < 0:
                _LOGGER.debug(
                    f"Cycle rejected: charge_month reset detected "
                    f"({charge_start:.1f} → {charge_end:.1f} Wh)"
                )
                return None

            # 3. Spočítat vybíjecí energii během intervalu
            total_discharge_wh = discharge_end - discharge_start

            # 4. Detekovat reset měsíce pro discharge
            if total_discharge_wh < 0:
                _LOGGER.debug(
                    f"Cycle rejected: discharge_month reset detected "
                    f"({discharge_start:.1f} → {discharge_end:.1f} Wh)"
                )
                return None

            # 5. KLÍČOVÁ PODMÍNKA: Žádné vybíjení během nabíjení
            # Tolerance: max 100 Wh vybíjení (např. drobné přepnutí režimu)
            max_allowed_discharge_wh = 100.0
            if total_discharge_wh > max_allowed_discharge_wh:
                _LOGGER.debug(
                    f"Cycle rejected: discharge during charging "
                    f"({total_discharge_wh:.1f} Wh > {max_allowed_discharge_wh:.1f} Wh)"
                )
                return None

            # 6. Validace nabíjecí energie
            if total_charge_wh <= 0:
                _LOGGER.debug(f"Invalid charge energy: {total_charge_wh:.1f} Wh")
                return None

            _LOGGER.debug(
                f"Clean charging cycle: charge={total_charge_wh:.1f} Wh, "
                f"discharge={total_discharge_wh:.1f} Wh"
            )

            # 7. Výpočet kapacity: energie / delta_soc
            # Pro čisté nabíjení neodečítáme discharge (je ~0)
            measured_capacity_kwh = (total_charge_wh / 1000.0) / (delta_soc / 100.0)
            soh_percent = (measured_capacity_kwh / self._nominal_capacity) * 100.0

            # Sanity check: odmítnout nereálné kapacity
            if (
            ...
            # Purity = 100% protože jsme ověřili že discharge ≈ 0
            purity = (
                1.0
                if total_discharge_wh == 0
                else (total_charge_wh / (total_charge_wh + total_discharge_wh))
            )

            # Quality score - zjednodušený
            ...
            # Pro čisté nabíjecí cykly neaplikujeme quality threshold
            # Stačí že prošly všemi kontrolami výše
            ...
            measurement = BatteryMeasurement(
                ...
                duration_hours=duration_hours,
                purity=purity,
                interruption_count=0,  # Čisté nabíjení = 0 interrupcí
                quality_score=quality_score,
            )

            _LOGGER.info(
                f"✅ Pure charging cycle: {start_soc:.1f}%→{end_soc:.1f}% ({delta_soc:.1f}%), "
                f"charge={total_charge_wh:.0f} Wh, capacity={measured_capacity_kwh:.2f} kWh, "
    ...
    def analyze_degradation_trend(
        ...
        # Sanity check: pokud je degradace nerealistická, data jsou nespolehlivá
        if degradation_percent_per_year > self._max_degradation_percent_per_year:
            _LOGGER.warning(
                f"Unrealistic degradation rate: {degradation_percent_per_year:.1f}%/year "
                f"(max allowed: {self._max_degradation_percent_per_year}%). "
                f"Insufficient or inconsistent data."
            )
            return None
    ...
    def get_statistics_30d(self) -> Optional[Dict[str, Any]]:
        ...
        measurements_sorted = sorted(
            measurements, key=lambda m: m.timestamp, reverse=True
        )
    ...
    def calculate_degradation_trends(
        ...
        for months, label in [(3, "3_months"), (6, "6_months"), (12, "12_months")]:
            ...
            # Sanity check: příliš velká změna znamená nespolehlivá data
            # Pro 3 měsíce max ~4% degradace (15%/rok / 4)
            max_change_for_period = self._max_degradation_percent_per_year * (
                months / 12
            )
            if abs(capacity_change_percent) > max_change_for_period:
                _LOGGER.debug(
                    f"Skipping {label} trend: change {capacity_change_percent:.1f}% "
                    f"exceeds max {max_change_for_period:.1f}%"
                )
                trends[label] = None
                continue
    ...
    async def _load_cycles_from_storage(self) -> Optional[datetime]:
        ...
        try:
            ...
            for cycle_data in cycles:
                ...
                measured_capacity_kwh = cycle_data["output_data"][
                    "measured_capacity_kwh"
                ]
                if (
                    soh_percent > self._max_soh_percent
