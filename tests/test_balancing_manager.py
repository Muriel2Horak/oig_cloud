...
class TestOpportunisticBalancing:
    ...
    @pytest.mark.asyncio
    async def test_triggers_on_high_soc(
        self, balancing_manager, mock_hass, mock_plan_manager
    ):
        ...
        balancing_manager._detect_holding_window = AsyncMock(
            return_value=holding_window
        )
...
class TestExplicitHoldingParameters:
    ...
    @pytest.mark.asyncio
    async def test_holding_parameters_in_plan(
        self, balancing_manager, mock_plan_manager
    ):
