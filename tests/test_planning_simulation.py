...
class TestModeSimulation:
    ...
    def test_home_iii_solar_to_battery_priority(self, simulation):
        ...
        assert result.grid_import_kwh == pytest.approx(0.75, abs=0.01)  # Consumption from grid
    ...
    def test_home_ups_grid_charging(self, simulation):
        ...
        assert result.grid_import_kwh > result.consumption_kwh  # Import for consumption + charging
...
class TestDeficitFix:
    ...
    def test_deficit_detection(self, simulation):
        ...
        assert result.is_deficit or result.battery_after_kwh < simulation.context.min_capacity_kwh
