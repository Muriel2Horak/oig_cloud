...
class _Registry:
    ...
    def async_get_device(self, identifiers):
        return self._devices.get(next(iter(identifiers)))
...
def test_children_are_linked_to_the_box_device(monkeypatch):
    ...
    linked = async_link_child_devices(object(), "2206237016")
...
def test_already_linked_child_is_left_alone(monkeypatch):
    ...
    assert async_link_child_devices(object(), "2206237016") == 0
...
def test_blank_box_id_is_a_no_op():
    assert async_link_child_devices(object(), "") == 0
