...
Compatibility and stability release for Home Assistant 2026.8. Every item below
was observed on a live install, not inferred from code review.
...
- **Blocking file read inside the event loop** — the shield telemetry module read
  `manifest.json` at import time; the version is now resolved lazily off-loop.
