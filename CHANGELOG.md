...
### Fixed
- ServiceShield boiler confirmation now resolves the target `boiler_manual_mode` entity from the service call `device_id`, preventing false pending/timeout states when a different box is selected than the config entry default.
